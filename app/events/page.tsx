...
function EventsContent() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [search, category, status, priceFilter, sortBy]);
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      {hasFilters && (
        <div className="flex flex-wrap items-center gap-2 mb-6">
          ...
          <button
            onClick={clearAll}
            className="text-xs text-coral-500 font-semibold hover:text-coral-700 transition-colors"
          >
            Clear all
          </button>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        <div className="animate-pulse space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-8">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-64 bg-teal-50 rounded-2xl" />
            ))}
          </div>
        </div>
      </div>
